...
class ThemeBloc extends Bloc<ThemeEvent, ThemeState> {
  ...
  _onChangeThemeMode(ChangeThemeMode event, Emitter<ThemeState> emit) async {
    await MySharedPreferences.instance.storeTheme(event.selectedTheme.themeMode);
    emit((state.copyWith(selectedTheme: event.selectedTheme)));
  }

  _onGetTheme(GetTheme event, Emitter<ThemeState> emit) async {
    final ThemeMode? selectedTheme = await MySharedPreferences.instance.getTheme();
    emit(state.copyWith(selectedTheme: Themes.values.where((element) => element.themeMode == selectedTheme).first));
  }
}
